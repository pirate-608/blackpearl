...
需要注意的是，项目使用的 OpenAI SDK 不只限于 OpenAI 官方模型。通过 `OPENAI_BASE_URL`，它可以连接实现 OpenAI-compatible API 的多厂商模型服务。兼容范围取决于目标服务是否支持 Responses API、function calling 和对应鉴权方式。
