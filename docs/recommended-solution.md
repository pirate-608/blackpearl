...
```text
+-------------------------------------------------------------+
| Session: agent-demo   Mode: agent   Model: $OPENAI_MODEL    |
+--------------------------+----------------------------------+
...
- 若厂商兼容 Responses API 与 function calling，使用 `OPENAI_API_MODE=responses`。
- 若厂商只兼容 Chat Completions，使用 `OPENAI_API_MODE=chat_completions`。
