...
1. 保存用户消息。
2. 调用 `ResponseRunner`。
3. 将 runner 产生的事件写入 session、event bus 和 transcript。
4. 保存最终回答。
5. 捕获并展示错误。
...
当前只有 `TranscriptStore`，以 JSONL 格式保存运行记录。JSONL 适合增量追加，也方便后续用脚本分析或生成 demo 材料。
...
用户输入
  ...
  -> OpenAI-compatible API
  -> final answer
  -> AgentSession / TranscriptStore / TUI
```
