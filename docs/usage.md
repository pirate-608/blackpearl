...
BLACKPEARL_API_KEY=你的 API Key
BLACKPEARL_BASE_URL=
BLACKPEARL_MODEL=账号可用的主 Agent 模型 ID
BLACKPEARL_SUBAGENT_MODEL=可选：多 Agent 使用的同厂商模型 ID
...
| `BLACKPEARL_API_KEY` | 是 | 无 | 当前 provider 使用的 API Key |
| `BLACKPEARL_BASE_URL` | 否 | provider 默认地址 | 模型服务 base URL，例如第三方网关或厂商兼容端点 |
| `BLACKPEARL_MODEL` | 否 | `gpt-4.1-mini` | 主 Agent 调用的模型 |
| `BLACKPEARL_SUBAGENT_MODEL` | 否 | 同主模型 | 多 Agent 规划、执行、汇总阶段使用的模型；当前要求与主模型同 provider、同 base URL |
| `BLACKPEARL_API_MODE` | 否 | `responses` | API 适配模式：`responses` 或 `chat_completions` |
| `AGENT_MAX_STEPS` | 否 | `6` | 单次任务最多允许的模型/工具循环次数 |

`BLACKPEARL_MODEL` 建议按账号实际可用模型配置。课程演示时可以选用更强模型，日常开发可选低成本模型。

兼容说明：历史版本使用 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`、`OPENAI_API_MODE`。当前版本仍会把这些变量作为 fallback 读取，但新配置推荐统一使用 `BLACKPEARL_*` 命名，避免误以为项目只能连接 OpenAI 官方服务。
...
项目使用 OpenAI SDK，并通过 `BLACKPEARL_BASE_URL` 支持 OpenAI-compatible API。根据目标服务支持的接口选择 `BLACKPEARL_API_MODE`：
...
```text
BLACKPEARL_API_KEY=第三方服务密钥
BLACKPEARL_BASE_URL=https://example-provider.test/v1
BLACKPEARL_MODEL=provider-model-name
BLACKPEARL_SUBAGENT_MODEL=provider-smaller-or-cheaper-model
BLACKPEARL_API_MODE=chat_completions
```

DeepSeek 示例：

```text
BLACKPEARL_PROVIDER=deepseek
BLACKPEARL_API_KEY=你的 DeepSeek API Key
BLACKPEARL_BASE_URL=https://api.deepseek.com/anthropic
BLACKPEARL_MODEL=deepseek-v4-pro
BLACKPEARL_SUBAGENT_MODEL=deepseek-v4-pro
BLACKPEARL_API_MODE=chat_completions
```
...
- 如果厂商只兼容 Chat Completions，而不兼容 Responses API，请使用 `BLACKPEARL_API_MODE=chat_completions`。DeepSeek 是例外：默认走 Anthropic-compatible endpoint。
...
```text
查看 src/tools 目录，搜索 file_write 的实现，然后说明当前文件写入安全策略
```
